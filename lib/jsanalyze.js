...
exports.analyzeJs = function analyzeJs(contents, opts) {
	...
	// minify
	if (opts.minify && !opts.filename.endsWith('.min.js')) {
		var UglifyJS = require("uglify-es");
		// FIXME we can't re-use the ast here, because we traversed it
		let data = {};
		data[opts.filename] = contents;
		// console.log('minify', data);
		var result = UglifyJS.minify(data, opts.minifyOptions || {mangle:{
			safari10: true
		}});
		if (!result.error) {
			results.contents = result.code;
		} else {
			throw result.error;
	}
