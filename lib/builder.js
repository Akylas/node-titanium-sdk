...
Builder.prototype.copyDirSync = function copyDirSync(src, dest, opts) {
	...
	(function copy(src, dest, isRootDir) {
		...
		fs.readdirSync(src).forEach(function (name) {
			...
			if (opts.maintainSymLinks && srcStat.isSymbolicLink()) {
				console.log('copyDirSync', src, srcFile, opts);
				var origStat = fs.lstatSync(fs.realpathSync(srcFile));
...
Builder.prototype.copyOrSymlinkDirSync = function copyOrSymlinkDirSync(src, dest, opts) {
	if (!fs.existsSync(src)) return;

	opts && typeof opts === 'object' || (opts = {});

	if (opts.forceCopy || !this.symlinkFilesOnCopy || opts.contents) {
		(function copy(src, dest, isRootDir) {
			fs.existsSync(dest) || wrench.mkdirSyncRecursive(dest);
			fs.readdirSync(src).forEach(function (name) {
				var srcFile = path.join(src, name),
	...
	} else if (!fs.existsSync(dest)|| (fs.lstatSync(dest).isSymbolicLink() && fs.realpathSync(dest) !== src)) {
...
Builder.prototype.validateTiModules = function validateTiModules(platformName, deployType, callback) {
	...
	appc.timodule.find(this.cli.tiapp.modules, platformName, deployType, ti.manifest, moduleSearchPaths, this.config, this.logger, function (modules) {
